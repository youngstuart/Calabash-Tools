...
from . import pipeman_ui as ui_file
from . import openAsset as oa
from . import refEdit
from . import arborist
from calabash import fileUtils
importlib.reload(ui_file)
importlib.reload(oa)
importlib.reload(refEdit)
importlib.reload(arborist)
...
class myGui(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    ...
    def open_linksUI(self):
        debug = False
        from . import gondo_link as glink
        importlib.reload(glink)
    ...
    def gondo_popshots(self):
        ...
        gshots = list(self.getRenDirs().keys())
    ...
    def gondo_poplayers(self):
        ...
        for layer in list(glayers.keys()):
    ...
    def isSpot(self, dir):
        ...
        if os.path.isdir(dir):
            if [x for x in os.listdir(dir) if re.match('sh[0-9]+', x)]:
                return True
            else:
                return False
        ...
        for subname in subnames:
            try:
                ...
                for f in files:
                    ...
                    if subname in f:
                        if not subname in list(version_dict.keys()):
            ...
            if asset.childCount() < 1:
                ...
                if len(latest) > 1:
                    ...
                    try:
                        ...
                        if debug: print('#open_latest# latest.values()', list(latest.values()))
                        path = latest[list(latest.keys())[0]]
                        print('Opening:', path)
                        pm.openFile(path)
    ...
    def retrofit(self):
        debug = False
        from . import retrofit as retrofit
        importlib.reload(retrofit)
